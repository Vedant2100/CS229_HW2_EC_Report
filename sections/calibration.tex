...
\begin{itemize}
    ...
    \item \textbf{Mixup} (train-time): Trains on convex combinations of input pairs $\tilde{x} = \lambda x_i + (1-\lambda)x_j$ with blended labels, where $\lambda \sim \text{Beta}(\alpha, \alpha)$ \cite{zhang2018mixup}. This forces intermediate confidences. We tested $\alpha \in \{0.1, 0.2, 0.4\}$.
\end{itemize}
